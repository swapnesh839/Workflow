...
//   const [isAdding, SetisAdding] = useState(false);
//   const [preview, setPreview] = useState(null);
//   const [NewRecipe, SetNewRecipe] = useState({ name: "Recipe 1", config: { width: "400", height: "70", x: 0, y: 60, text: "" } });
...
//       const data = await getAllData({ storeName: "Recipes" });
//       SetRecipes(data || []);
//       SetNewRecipe({ name: `Recipe ${data?.length + 1 || 1}`, config: { width: "400", height: "70", x: 0, y: 60, text: "" } });
//     } catch (error) {
//       console.error('Error fetching recipes:', error);
//     }
//   };

//   const AddRecipes = (alert) => {
//     const newRecipe = { name: `Recipe ${Recipes.length + 1}`, config: { width: "400", height: "70", x: 0, y: 60, text: alert } };
//     SetNewRecipe(newRecipe);
//     SetLocalChanges(prev => [...prev, newRecipe]);
//     SetisAdding(true);
//   };
...
//         await addData({ storeName: "Recipes", newData: recipe });
//       }
//       SetFetch(prev => !prev);
//       setLocalChanges([]);
//       SetisAdding(false);
...
//   const deleteLastConfig = () => {
//     const updatedRecipes = [...Recipes];
//     updatedRecipes.pop();
//     SetRecipes(updatedRecipes);
//     setLocalChanges(updatedRecipes);
//   };
...
//     const updatedRecipes = Recipes.map(recipe => recipe.id === id ? updatedData : recipe);
//     SetRecipes(updatedRecipes);
//     setLocalChanges(updatedRecipes);
//   };
...
//       SetRecipes(prev => {
//         const updatedRecipes = [...prev];
//         updatedRecipes[ActiveRecipe] = { ...updatedRecipes[ActiveRecipe], name: newName };
//         return updatedRecipes;
//       });
//     } else if (isAdding) {
//       SetNewRecipe(prev => ({ ...prev, name: newName }));
//     }
//   };
...
//                       <div className='bg-dark text-white p-2 mb-2 rounded-3 d-flex'>
//                         <span className='me-auto my-auto'>{NewRecipe.name}</span>
//                         <span onClick={saveChangesToDB} className='my-auto ms-auto bg-success rounded-1 py-1 px-2 cursor-pointer'>Save New Recipe</span>
//                       </div>
//                     )}
//                     {(isAdding || preview !== null) && (
//                       Recipes?.map((i, index) => (
//                         <Component key={index} index={index} config={i.config} setActiveRecipe={setActiveRecipe} deleteRecipe={deleteRecipe} isLast={index === Recipes.length - 1} text={i.name} ActiveRecipe={ActiveRecipe} />
//                       ))
//                     )}
//                     {(!isAdding && preview === null) && (
//                       <div className='h-100 w-100 d-flex align-items-center justify-content-center text-white'>
...
//               <div style={{ height: "500px", width: "400px" }} className='rounded-3 bg-dark d-inline-block p-0'>
//                 <div className='p-2'>
//                   <Select onChange={handleSelectChange} className='text-black' options={options} value={Selectedoption} />
//                   <Row className='mt-3'>
//                     {Selectedoption?.Alerts?.map((alert, index) => (
//                       <Col className='' key={index} xxl="6" xl="6" md="6" sm="6">
//                         <div onClick={() => { AddRecipes(alert) }} className='bg-light text-black p-2 mb-2 rounded cursor-pointer'>
...
//             <p className='border-bottom border-dark border-2 w-100 p-2 sticky-top z-3 text-white'>Recipes</p>
//             {preview !== null && (
//               <Button className='position-absolute top-0 end-0 m-1 z-3' onClick={clearRecipeSelected}> Clear selected Recipe </Button>
//             )}
//             <Row className=' h-100 w-100 p-2'>
//               {Recipes?.map((i, index) => (
//                 <Col key={index} lg="3" md="3" sm="3" xs="3" xxl="3" onClick={() => { setPreview(index) }} className="text-center p-2 cursor-pointer">
//                   <div className={`p-3 bg-dark-subtle position-relative text-dark ${preview === index && "border border-info border-2"}`}>
//                     <Trash onClick={(e) => { e.stopPropagation(); deleteRecipe(i.id); }} className='position-absolute top-0 end-0 bg-danger text-white rounded-2 p-1 m-1' />
//                     {i.name}
//                   </div>
//                 </Col>
//               ))}
...
// const Component = ({ index, config, setActiveRecipe, deleteRecipe, isLast, text, ActiveRecipe }) => {
//   const [state, setState] = useState({ width: config?.width || 400, height: config?.height || 70, x: config?.x || 0, y: config?.y || 0, text: config?.text || "" });
//   const handleFocus = (id) => {
//     setActiveRecipe(id);
//   }
//   return (
...
//       onResizeStop={(e, direction, ref, delta, position) => { setState((prev) => ({ ...prev, width: ref.style.width, height: ref.style.height })); }}
//       minWidth={100}
//       minHeight={50}
//       bounds="parent"
//       className='text-dark text-center bg-dark-subtle d-flex rounded-2'
